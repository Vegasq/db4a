...
1. **The cartridge is committed, deliberately.** Decided 2026-08-25, reversing
   the rule that stood here before. `roms/` holds the one canonical dump because
   the build translates it into C and cannot run without it -- not from a clean
   checkout, not in CI. `.gitignore` still excludes everything ELSE under
   `roms/`, so a stray or alternate dump cannot be swept in by `git add -A`.
   Everything DERIVED from the cartridge -- framebuffers, RAM dumps, save
   states, audio captures, packaged builds -- stays out and stays gitignored.
   README's Legal section names the rights holders and commits to removal on
   request.
...
roms/                the cartridge, committed (see Ground rules #1)
```
